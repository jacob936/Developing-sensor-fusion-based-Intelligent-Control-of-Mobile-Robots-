#!/usr/bin/env python3
"""
ASMC Controller - Simple Version with Waypoint Bug Fix Only
"""
...
class AdaptiveSMCController(Node):
    def __init__(self):
        ...
        self.csv_file = f'{self.output_dir}/asmc_{timestamp}.csv'
        ...
        self.get_logger().info('ASMC Controller - Simple Version')
        self.get_logger().info(f'kp_v={self.kp_linear}, kp_w={self.kp_angular}')
    ...
    def advance_waypoint(self, distance):
        if distance < self.waypoint_threshold:
            ...
            # FIXED: Properly wrap waypoint index
    ...
    def get_current_waypoint(self):
        """FIXED: Safely get current waypoint with bounds checking"""
        if self.reference_path is None or len(self.reference_path) == 0:
            return None
    ...
    def compute_control(self):
        ...
        # FIXED: Use safe waypoint accessor
        ...
        # Recompute after waypoint advance
        current_wp = self.get_current_waypoint()
        if current_wp is None:
            return 0.0, 0.0
